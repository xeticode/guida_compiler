...
if (fs.existsSync(minPath)) {
  ...
  const minHasWasm = minCode.includes('WASM-ACCELERATED RUNTIME') || minCode.includes('_WASM_INITIALIZED');
...
try {
  ...
  if (typeof _WASM_INITIALIZED !== 'undefined') {
    console.log(`   WASM initialized: ${_WASM_INITIALIZED ? 'YES' : 'NO (fallback to JS)'}`);
  }
  ...
  if (typeof _String_reverse === 'function') {
    ...
    if (reversed === 'olleh') {
      ...
      console.error(`   ✗ _String_reverse("hello") = "${reversed}" (expected "olleh")`);
      process.exit(1);
    }
  }
} catch (e) {
